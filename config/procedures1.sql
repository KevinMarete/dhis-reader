...
BEGIN
	IF (ordcode = 'F-CDRR') THEN
		...
		/*Save Logs of rejected cdrrs*/
		REPLACE INTO tbl_cdrr_log_temp (code, period_begin, period_end, facility_id, description, created, user_id)
		SELECT c.code, c.period_begin, c.period_end, c.facility_id, cl.description, cl.created, cl.user_id
		...
		/*Upsert cdrr_logs for rejected cdrrs*/
		REPLACE INTO tbl_cdrr_log (description, created, user_id, cdrr_id)
		SELECT clt.description, clt.created, clt.user_id, c.id cdrr_id
		FROM tbl_cdrr_log_temp clt 
		INNER JOIN tbl_cdrr c ON c.code = clt.code AND c.period_begin = clt.period_begin AND c.period_end = clt.period_end AND c.facility_id = clt.facility_id;

		/*Update cdrr status to rejected*/
		UPDATE tbl_cdrr c
		INNER JOIN tbl_cdrr_log_temp clt ON c.code = clt.code AND c.period_begin = clt.period_begin AND c.period_end = clt.period_end AND c.facility_id = clt.facility_id
		SET status = 'rejected' 
		WHERE c.status IN ('pending');
		
		/*Clear temporary cdrr_log table*/
		/*TRUNCATE tbl_cdrr_log_temp*/;
...
BEGIN
	IF (ordcode = 'F-MAPS') THEN
		...
		/*Save Logs of rejected maps*/
		REPLACE INTO tbl_maps_log_temp (code, period_begin, period_end, facility_id, description, created, user_id)
		SELECT c.code, c.period_begin, c.period_end, c.facility_id, cl.description, cl.created, cl.user_id
		...
		/*Upsert cdrr_logs for rejected maps*/
		REPLACE INTO tbl_maps_log (description, created, user_id, cdrr_id)
		SELECT clt.description, clt.created, clt.user_id, c.id cdrr_id
		FROM tbl_maps_log_temp clt 
		INNER JOIN tbl_cdrr c ON c.code = clt.code AND c.period_begin = clt.period_begin AND c.period_end = clt.period_end AND c.facility_id = clt.facility_id;

		/*Update cdrr status to rejected*/
		UPDATE tbl_maps c
		INNER JOIN tbl_maps_log_temp clt ON c.code = clt.code AND c.period_begin = clt.period_begin AND c.period_end = clt.period_end AND c.facility_id = clt.facility_id
		SET status = 'rejected' 
		WHERE c.status IN ('pending');
		
		/*Clear temporary cdrr_log table*/
		TRUNCATE tbl_maps_log_temp;
    ELSE
    	/*Drop unique index*/
		ALTER TABLE tbl_order DROP INDEX facility_period_dimension_category;
...
BEGIN
    ...
    /*dsh_consumption*/
    REPLACE INTO dsh_consumption(total, allocated, data_year, data_month, data_date, sub_county, county, facility, drug,partner)
    SELECT 
    ...
    /*dsh_stock*/
    REPLACE INTO dsh_stock(total, amc_total, data_year, data_month, data_date, sub_county, county, facility, drug,partner)
    SELECT 
    ...
    /*dsh_order*/
    REPLACE INTO dsh_order(data_year, data_month, data_date, sub_county, county, facility, facility_category,partner)
    SELECT
    ...
    /*dsh_order_item*/
    REPLACE INTO dsh_order_item(opening_bal_qty, received_qty, consumed_qty, losses_qty, adjustment_qty, closing_bal_qty, data_year, data_month, data_date, sub_county, county, facility, facility_category, drug,parter)
    SELECT
